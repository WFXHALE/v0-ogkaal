...
function WaterDropIntro({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    return () => { clearTimeout(safety); clearTimeout(t1); clearTimeout(t2); clearTimeout(t3) }
  }, [finish])

  if (phase === "done") return null

  const textMap: Record<DropPhase, string> = { drop1: "OG", drop2: "KAAL", drop3: "WELCOME KAAL", done: "" }

  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "#000", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "2rem" }}>
      <style>{DROP_KEYFRAMES}</style>
      <div
        key={phase + "-dot"}
        style={{ width: 14, height: 14, borderRadius: "50%", background: "#FCD535", boxShadow: "0 0 22px 7px rgba(252,213,53,0.45)", animation: dotAnim, willChange: "transform, opacity", flexShrink: 0 }}
      />
      <div style={{ position: "relative", height: "5rem", minWidth: "280px", textAlign: "center" }}>
        <span
          key={phase + "-text"}
          style={{ position: "absolute", left: "50%", top: 0, transform: "translateX(-50%)", whiteSpace: "nowrap", fontFamily: "var(--font-sans), system-ui, sans-serif", fontWeight: 900, fontSize: "clamp(2rem, 8vw, 3.5rem)", letterSpacing: "0.18em", color: "#FCD535", animation: "wtext-in 0.35s cubic-bezier(0.22,1,0.36,1) forwards", willChange: "opacity, transform, filter" }}
        >
          {textMap[phase]}
        </span>
      </div>
      <p style={{ fontFamily: "var(--font-sans), system-ui, sans-serif", fontSize: "0.62rem", letterSpacing: "0.38em", textTransform: "uppercase", color: "rgba(252,213,53,0.35)", whiteSpace: "nowrap", opacity: phase === "drop3" ? 1 : 0, animation: phase === "drop3" ? "wsub-in 0.5s ease forwards" : "none" }}>
...
function CountdownBadge({ onExpire }: { onExpire: () => void }) {
  ...
  return (
    <div style={{ position: "fixed", top: 18, right: 18, zIndex: 300, display: "flex", alignItems: "center", gap: "0.4rem", padding: "0.35rem 0.75rem", borderRadius: "999px", background: urgent ? "rgba(239,68,68,0.12)" : "rgba(252,213,53,0.08)", border: `1px solid ${urgent ? "rgba(239,68,68,0.35)" : "rgba(252,213,53,0.25)"}`, transition: "background 0.3s, border-color 0.3s", animation: urgent ? "timer-pulse 1s ease-in-out infinite" : "none" }}>
      <style>{`@keyframes timer-pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.6; } }`}</style>
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={urgent ? "#ef4444" : "#FCD535"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
      </svg>
      <span style={{ fontSize: "0.7rem", fontWeight: 700, letterSpacing: "0.06em", color: urgent ? "#ef4444" : "#FCD535", fontFamily: "monospace", transition: "color 0.3s" }}>
        {timeLeft}s
      </span>
    </div>
  )
    ...
    <div className="min-h-screen bg-black flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-10">
          <p className="text-3xl font-bold tracking-widest text-[#FCD535] uppercase">OG KAAL</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-500 pointer-events-none">
