    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md">
        ...
        <div className="p-6 rounded-2xl bg-card border border-border shadow-xl">
          ...
          {/* Demo reset code hint */}
          {demoCode && (
            <div className="flex items-center gap-2 p-3 rounded-xl bg-primary/10 border border-primary/30 mb-4">
              <Lock className="w-4 h-4 text-primary shrink-0" />
              <p className="text-sm text-primary">Reset code: <span className="font-mono font-bold">{demoCode}</span></p>
            </div>
          )}
          ...
          {screen === "forgot" && (
            <form onSubmit={handleForgot} className="space-y-4">
              ...
              <button type="button" onClick={() => { clear(); setScreen("login") }} className="w-full text-xs text-muted-foreground hover:text-foreground text-center">
                Back to login
              </button>
            </form>
          )}
