...
import {
  ...
  Globe, ToggleLeft, ToggleRight, Mail, Phone, Edit,
  ExternalLink,
} from "lucide-react"
...
const DEMO_SUBMISSIONS: Submission[] = [
  ...
  },
  {
    ...
    utr: "abc123txid", screenshotUrl: "", ipAddress: "182.73.45.12", location: "Delhi, India",
...
export default function AdminDashboardPage() {
  ...
  const allFiles = [
    ...submissions.filter(s => s.screenshotUrl).map(s => ({
      id: s.id, userId: s.userId || "—", name: s.name, type: "Payment Screenshot",
      url: s.screenshotUrl!, createdAt: s.createdAt,
    })),
    ...usdtBuy.filter(r => r.screenshotUrl).map(r => ({
      id: r.id, userId: r.userId, name: r.name, type: "USDT Buy Proof",
      url: r.screenshotUrl!, createdAt: r.createdAt,
    })),
    ...usdtSell.filter(r => r.screenshotUrl).map(r => ({
      id: r.id, userId: r.userId, name: r.name, type: "USDT Sell Proof",
      url: r.screenshotUrl!, createdAt: r.createdAt,
    })),
  ]
              ...
              {filteredMembers.length === 0
                ...
                : filteredMembers.map((u, i) => (
                  <tr key={u.id} className="border-b border-border/50 hover:bg-secondary/20 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <button onClick={() => setDetailView(u)} className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-secondary" title="Full Profile"><Eye className="w-4 h-4" /></button>
                    </td>
                  </tr>
                ))
              }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {allFiles.map(f => (
            <div key={f.id} className="rounded-xl bg-card border border-border overflow-hidden">
              <div
                className="h-40 bg-secondary/50 flex items-center justify-center cursor-pointer hover:opacity-80 transition-opacity"
                onClick={() => setScreenshotModal(f.url)}
              >
                <img src={f.url} alt={f.type} className="h-full w-full object-cover" onError={e => { (e.target as HTMLImageElement).style.display = "none" }} />
    ...
    const exports = [
      {
        ...
        desc: "All registered members with contact info and membership status",
        count: userList.length,
        action: () => downloadCSV(userList.map(u => ({ userId: u.userId, name: u.name, email: u.email || "", phone: u.phone || "", telegram: u.telegram || "", type: u.type, status: u.status, joined: u.createdAt })), "members.csv"),
      },
      {
        label: "Payment Records",
        desc: "All payment submissions including status, amounts, and UTR numbers",
        count: submissions.length,
        action: () => downloadCSV(submissions.map(s => ({ id: s.id, userId: s.userId, name: s.name, method: s.paymentMethod, amount: s.amount, utr: s.utr, status: s.status, date: s.createdAt })), "payments.csv"),
      },
      {
        label: "USDT Buy Requests",
        desc: "All USDT buy requests with wallet addresses and transaction IDs",
        count: usdtBuy.length,
        action: () => downloadCSV(usdtBuy.map(r => ({ id: r.id, userId: r.userId, name: r.name, wallet: r.walletAddress, txId: r.txId, amountUsdt: r.amountUsdt, inr: r.inrEquivalent, status: r.status, date: r.createdAt })), "usdt-buy.csv"),
      },
      {
        label: "USDT Sell Requests",
        desc: "All USDT sell requests with UPI IDs for INR payouts",
        count: usdtSell.length,
        action: () => downloadCSV(usdtSell.map(r => ({ id: r.id, userId: r.userId, name: r.name, upi: r.upiId, wallet: r.walletAddress, usdt: r.usdtAmount, txId: r.txId, status: r.status, date: r.createdAt })), "usdt-sell.csv"),
      },
    ]
    return (
      <div className="space-y-4">
    ...
    <div className="space-y-6">
        ...
        {[
          ...
          <div key={key} className="flex items-center gap-4">
            ...
            <input
              ...
              onChange={e => setSystemSettings(s => ({ ...s, [key]: e.target.value }))}
              onBlur={() => saveSystem({})}
      ...
      <div className="rounded-xl bg-card border border-border p-5 space-y-3">
        ...
        <textarea
          ...
          onChange={e => setSystemSettings(s => ({ ...s, paymentInstructions: e.target.value }))}
          onBlur={() => saveSystem({})}
          rows={3}
          className="w-full px-3 py-2.5 rounded-lg bg-secondary border border-border text-sm text-foreground focus:outline-none focus:border-primary/50 resize-none"
        />
      </div>
  ...
  const renderSecurity = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        {secMsg && (
          <div className={`p-3 rounded-lg border text-sm ${secMsg.type === "ok" ? "bg-green-500/10 border-green-500/30 text-green-400" : "bg-red-500/10 border-red-500/30 text-red-400"}`}>
            {secMsg.text}
          </div>
        )}
        ...
        <div className="rounded-xl bg-card border border-border p-5 space-y-4">
          ...
          {[
            ...
            <div key={key} className="flex items-center gap-3">
              ...
              <input
                value={secForm[key]}
                onChange={e => setSecForm(f => ({ ...f, [key]: e.target.value }))}
                className="flex-1 px-3 py-2 rounded-lg bg-secondary border border-border text-sm text-foreground focus:outline-none focus:border-primary/50"
              />
            </div>
          ))}
        ...
        <Button onClick={handleSecSave} className="bg-primary text-primary-foreground hover:bg-primary/90 font-semibold">
          Save Changes
        </Button>
