...
export async function POST(req: NextRequest) {
  try {
    ...
    const user = await queryOne<{
      ...
      email: string
      full_name: string
      otp_attempts_today: number | null
      otp_attempts_reset_at: string | null
    }>(
      "SELECT id, email, full_name, otp_attempts_today, otp_attempts_reset_at FROM dashboard_users WHERE email = $1",
      [normalised]
    )
    ...
    const newResetAt = resetAt && (now - resetAt) < windowMs
      ? user.otp_attempts_reset_at
      : new Date(now).toISOString()

    await query(
      `UPDATE dashboard_users
       SET otp_hash = $1, otp_expires_at = $2, otp_attempts_today = $3, otp_attempts_reset_at = $4
       WHERE id = $5`,
      [otpHash, expiresAt, attemptsToday + 1, newResetAt, user.id]
    )
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const user = await queryOne<{
      ...
      "SELECT id, otp_hash, otp_expires_at FROM dashboard_users WHERE email = $1",
      [normalised]
    )
    ...
    // Hash using EXACT same logic as hashPassword() in dash-auth.ts
    ...
    await query(
      "UPDATE dashboard_users SET password_hash = $1, otp_hash = NULL, otp_expires_at = NULL WHERE id = $2",
      [pwHash, user.id]
    )

    return NextResponse.json({ success: true })
  } catch (err) {
    console.error("[forgot-password] PUT error:", err)
    return NextResponse.json({ error: "Server error." }, { status: 500 })
  }
}
