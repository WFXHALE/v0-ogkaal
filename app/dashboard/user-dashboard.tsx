        ...
        <div className="rounded-2xl border border-border bg-card p-6 space-y-5">
          ...
          {mode === "backup" && (
            <form onSubmit={handleBackupLogin} className="space-y-4">
              <p className="text-sm text-muted-foreground">Enter your email and 16-character backup code to regain access.</p>
              <div>
          ...
          {/* Backup code shown post-registration */}
          {mode === "backup_shown" && (
    ...
    <div className="rounded-xl border border-border bg-secondary/20 p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="p-2.5 rounded-lg bg-background border border-border">
          ...
          <p className={`text-sm font-semibold ${expiring ? "text-amber-400" : membership?.status === "expired" ? "text-red-400" : "text-foreground"}`}>
...
export default function UserDashboard() {
  ...
  // Load data when session established
  const loadData = useCallback(async (s: DashboardSession) => {
    ...
    const { data: mems } = await supabase
      .from("memberships")
      .select("*")
      .or(`email.eq.${s.email},user_id.eq.${s.id}`)
    ...
    const [{ data: buyData }, { data: sellData }] = await Promise.all([
      supabase.from("usdt_buy_requests").select("*").or(`email.eq.${s.email},user_id.eq.${s.id}`).order("created_at", { ascending: false }),
      supabase.from("usdt_sell_requests").select("*").or(`email.eq.${s.email},user_id.eq.${s.id}`).order("created_at", { ascending: false }),
    ])
    const combined = [
          ...
          >
            <div className="pt-1 space-y-3">
              {!hasVip ? (
                ...
                <>
                  {signals.map(s => (
                    <div key={s.id} className="flex items-center gap-3 px-4 py-3 rounded-xl border border-border bg-secondary/20 flex-wrap gap-y-1">
